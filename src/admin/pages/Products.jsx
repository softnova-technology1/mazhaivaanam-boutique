          ...
          <form onSubmit={handleSave} style={{ background: 'var(--bg-primary)', borderRadius: 12, padding: 32, border: '1px solid var(--border-color)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: 32 }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    ...
                    >
                      ...
                      <input id="sec1-upload" type="file" accept="image/*" style={{ display: 'none' }} onClick={e => { e.stopPropagation(); e.target.value = null; }} onChange={e => {
                        const file = e.target.files[0];
                        if (!file) return;
                        if (file.size > 5 * 1024 * 1024) {
                          showToast('Image file size must be less than 5MB (suitable for S3 bucket)', 'error');
                    ...
                    >
                      ...
                      <input id="sec2-upload" type="file" accept="image/*" style={{ display: 'none' }} onClick={e => { e.stopPropagation(); e.target.value = null; }} onChange={e => {
                        const file = e.target.files[0];
                        if (!file) return;
                        if (file.size > 5 * 1024 * 1024) {
                          showToast('Image file size must be less than 5MB (suitable for S3 bucket)', 'error');
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {/* SKU Display — read only, auto-generated by server */}
                {modal.product?.sku && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: 'rgba(200,163,77,0.07)', borderRadius: 8, border: '1px solid rgba(200,163,77,0.25)' }}>
                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 500 }}>SKU</span>
                    <span style={{ fontFamily: 'monospace', fontWeight: 700, color: 'var(--primary)', fontSize: '0.9rem', letterSpacing: '0.08em' }}>
                      {modal.product.sku}
                    </span>
                    {modal.product.patternCode && (
                      <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginLeft: 'auto' }}>
                        Pattern {modal.product.patternCode} · #{modal.product.patternSeq}
                      </span>
                    )}
                  </div>
                )}
