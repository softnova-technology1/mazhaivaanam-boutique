...
export default function PreBooking() {
  ...
  const handleFileSelect = async (e) => {
    ...
    try {
      ...
      // Ensure items are marked as preorder
      const preorderItems = parsed.map(item => ({ ...item, isPreorder: true }));
      setImportModal(m => ({
        ...m,
        file,
        parsedProducts: preorderItems,
        error: '',
      }));
    } catch (err) {
      setImportModal(m => ({
  ...
  const handleExecuteImport = async () => {
    ...
    try {
      ...
      showToast(res.message || `Imported ${res.data?.importedCount || importModal.parsedProducts.length} pre-booking products successfully!`, 'success');
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                    ...
                    >
                      ...
                      <input id="preorder-sec1-upload" type="file" accept="image/*" style={{ display: 'none' }} onClick={e => { e.stopPropagation(); e.target.value = null; }} onChange={e => {
                        const file = e.target.files[0];
                        if (!file) return;
                        if (file.size > 5 * 1024 * 1024) {
                          showToast('Image file size must be less than 5MB (suitable for S3 bucket)', 'error');
                    ...
                    >
                      ...
                      <input id="preorder-sec2-upload" type="file" accept="image/*" style={{ display: 'none' }} onClick={e => { e.stopPropagation(); e.target.value = null; }} onChange={e => {
                        const file = e.target.files[0];
                        if (!file) return;
                        if (file.size > 5 * 1024 * 1024) {
                          showToast('Image file size must be less than 5MB (suitable for S3 bucket)', 'error');
          ...
          <div className="modal-content" style={{ maxWidth: 850, width: '90vw', maxHeight: '90vh', overflowY: 'auto', padding: 24, borderRadius: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 600 }}>Import Pre-Booking Products (Excel / CSV)</h3>
                  <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                    Upload a .csv or Excel file to batch insert multiple pre-reservation products
                  </p>
                </div>
              </div>
              <button
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  <h4 style={{ margin: 0, fontSize: '1rem', fontWeight: 600 }}>
                    Preview Pre-Order Products ({importModal.parsedProducts.length} rows found)
                  </h4>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
              ...
              <button
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 6 }}
              >
                {importModal.importing ? 'Importing Products...' : `Import ${importModal.parsedProducts.length} Pre-Orders Now`}
              </button>
            </div>
          </div>
        </div>
