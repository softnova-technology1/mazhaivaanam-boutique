          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2px solid #C8A34D', paddingBottom: 20 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: '0.85rem', color: '#475569', lineHeight: 1.6 }}>
                ...
                <div>Status: <strong style={{ color: (order.paymentStatus === 'paid' || order.status === 'DELIVERED') ? '#16a34a' : '#ea580c' }}>{(order.paymentStatus || 'PAID').toUpperCase()}</strong></div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, margin: '24px 0', padding: '16px 20px', background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' }}>
            <div>
              ...
              <div style={{ fontSize: '0.88rem', fontWeight: 600, color: '#0f172a' }}>
                {order.shippingAddress?.fullName || order.fullName || `${order.user?.firstName || ''} ${order.user?.lastName || ''}`.trim() || 'Valued Atelier Client'}
              </div>
              <div style={{ fontSize: '0.82rem', color: '#475569', lineHeight: 1.5, marginTop: 4 }}>
                Email: {order.user?.email || order.shippingAddress?.email || order.email || 'N/A'}<br />
                Phone: {order.shippingAddress?.phone || order.phone || order.user?.phone || 'N/A'}
              </div>
            </div>
